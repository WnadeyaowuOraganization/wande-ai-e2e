...
// 已认证访问测试
test.describe('合同管理 API 测试', () => {
  test('应能访问合同列表', async ({ request }) => {
    const response = await request.get(`${BASE_URL}/wande/contract/list`, {
  ...
  test('应能访问合同详情', async ({ request }) => {
    ...
    const listResponse = await request.get(`${BASE_URL}/wande/contract/list`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    if (listData.data && listData.data.rows && listData.data.rows.length > 0) {
      ...
      const response = await request.get(`${BASE_URL}/wande/contract/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      expect(response.status()).toBe(200);
      const data = await response.json();
  ...
  test('应能生成合同编号', async ({ request }) => {
    const response = await request.get(`${BASE_URL}/wande/contract/generate-number?contractType=direct`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    expect(response.status()).toBe(200);
    const data = await response.json();
  ...
  test('应能获取所有合同（不分页）', async ({ request }) => {
    const response = await request.get(`${BASE_URL}/wande/contract/all`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    expect(response.status()).toBe(200);
    const data = await response.json();
