/**
 * Feedback Management Page Tests (Cockpit version)
 * Issue: WnadeyaowuOraganization/wande-ai-front#63
 * PR: WnadeyaowuOraganization/wande-ai-front#343
 */
...
test.describe('Feedback Manage Page — API @smoke @feedback @issue:front#63', () => {
  ...
  test('feedback stats API returns data with valid token', async ({ request }) => {
    test.skip(!token, 'No token available');
    const response = await request.get(`${API_BASE}/wande/user-feedback/stats`, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
test.describe('Feedback Manage Page — Frontend @smoke @feedback @issue:front#63', () => {
  ...
  test('page loads without crash', async ({ page }) => {
    ...
    await page.goto('/super-admin/feedback-manage');
    await page.waitForLoadState('networkidle');
    expect(page.url()).toContain('feedback-manage');
  });
