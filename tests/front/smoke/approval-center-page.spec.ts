...
test.describe('Approval Center Page @smoke @approval-center @issue:front#33', () => {
  test('page loads without critical errors when menu is registered', { tag: ['@smoke', '@approval-center'] }, async ({ page, request }) => {
    ...
    await page.goto(`${FRONT_BASE}/wande/common/approval`);
  ...
  test('page has no critical console errors', { tag: ['@smoke', '@approval-center'] }, async ({ page, request }) => {
    const token = await getAuthToken(request);
    test.skip(!token, 'Login failed');
