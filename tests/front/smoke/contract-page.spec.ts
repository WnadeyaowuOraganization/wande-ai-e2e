/**
 * Contract Page Smoke Test
 * Issue: WnadeyaowuOraganization/wande-ai-front#85
 *
 * Page: /wande/common/contract
 */
...
test.describe('Contract Page @smoke @contract @issue:front#85', () => {
  test('page loads without critical errors when menu is registered', { tag: ['@smoke', '@contract'] }, async ({ page, request }) => {
    ...
    await page.goto(`${FRONT_BASE}/wande/common/contract`);
  ...
  test('page has no critical console errors', { tag: ['@smoke', '@contract'] }, async ({ page, request }) => {
    const token = await getAuthToken(request);
    test.skip(!token, 'Login failed');
