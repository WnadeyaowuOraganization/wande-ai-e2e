/**
 * Execution Page Smoke Test
 * Issue: WnadeyaowuOraganization/wande-ai-front#85
 *
 * Page: /wande/common/execution
 */
...
test.describe('Execution Page @smoke @execution @issue:front#85', () => {
  test('page loads without critical errors when menu is registered', { tag: ['@smoke', '@execution'] }, async ({ page, request }) => {
    ...
    await page.goto(`${FRONT_BASE}/wande/common/execution`);
  ...
  test('page has no critical console errors', { tag: ['@smoke', '@execution'] }, async ({ page, request }) => {
    const token = await getAuthToken(request);
    test.skip(!token, 'Login failed');
